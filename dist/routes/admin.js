...
exports.getClinicAuditLog = getClinicAuditLog;
...
router.post('/users/:id/approve', async (req, res) => {
  ...
  `;
    ...
    if (!existing.profile_slug && ['clinic_admin', 'doctor'].includes(existing.role)) {
        ...
        let n = 2;
        while (true) {
            const taken = await (0, db_1.default) `SELECT id FROM users WHERE profile_slug = ${slug}`;
            if (!taken.length)
                break;
            slug = `${base}-${n++}`;
        }
        await (0, db_1.default) `UPDATE users SET profile_slug = ${slug}, approved_at = NOW(), approval_status = 'approved' WHERE id = ${userId}`;
    }
    else {
        await (0, db_1.default) `UPDATE users SET approval_status = 'approved', approved_at = NOW() WHERE id = ${userId}`;
    }
    if (existing.email && ['clinic_admin', 'doctor'].includes(existing.role)) {
        const mail = (0, mailer_1.approvalEmail)(existing.name);
        (0, mailer_1.sendMail)(existing.email, mail.subject, mail.html);
    }
    res.json({ ok: true });
});
...
router.post('/users/:id/suspend', async (req, res) => {
    await (0, db_1.default) `UPDATE users SET approval_status = 'suspended' WHERE id = ${req.params.id}`;
    res.json({ ok: true });
});
...
router.post('/users/:id/unblock', async (req, res) => {
    ...
    await (0, db_1.default) `UPDATE users SET approval_status = 'approved' WHERE id = ${req.params.id}`;
...
router.get('/stats', async (_req, res) => {
    ...
    const [patients] = await (0, db_1.default) `SELECT COUNT(*) as total FROM patients`;
    const [visits] = await (0, db_1.default) `SELECT COUNT(*) as total FROM visits`;
    const [bookings] = await (0, db_1.default) `
    SELECT COUNT(*) as total,
           SUM(CASE WHEN status='pending' THEN 1 ELSE 0 END) as pending,
           SUM(CASE WHEN created_at > NOW() - INTERVAL '7 days' THEN 1 ELSE 0 END) as this_week
    FROM booking_requests`;
    const [logins] = await (0, db_1.default) `
    SELECT COUNT(*) as total,
...
router.get('/doctors/overview', async (_req, res) => {
    const rows = await (0, db_1.default) `
    SELECT
...
router.patch('/doctors/:id/directory', async (req, res) => {
    if (req.user.role !== 'superadmin') {
        res.status(403).json({ error: 'Forbidden' });
        return;
    }
    const { show } = req.body;
    await (0, db_1.default) `UPDATE users SET show_in_directory = ${show} WHERE id = ${req.params.id}`;
...
router.get('/funnel', async (_req, res) => {
    const [events, logins] = await Promise.all([
      ...
      ORDER BY count DESC
    `,
        (0, db_1.default) `SELECT COUNT(*) AS count FROM login_sessions WHERE logged_in_at > NOW() - INTERVAL '30 days'`,
    ]);
...
router.get('/failed-logins', async (_req, res) => {
    const rows = await (0, db_1.default) `
    SELECT
      metadata->>'email' AS email,
      metadata->>'method' AS method,
...
router.get('/recent-logins', async (_req, res) => {
    const rows = await (0, db_1.default) `
    SELECT
...
router.get('/geo-summary', async (_req, res) => {
    const byCityState = await (0, db_1.default) `
    SELECT
...
router.get('/geo-by-state', async (_req, res) => {
  ...
  `;
    ...
    const locations = await (0, db_1.default) `
    SELECT
      COALESCE(NULLIF(u.state, ''), NULLIF(u.reg_state, ''), 'Unknown') AS state,
...
router.delete('/email-templates/:id', async (req, res) => {
    await (0, db_1.default) `DELETE FROM email_templates WHERE id = ${req.params.id}`;
    res.json({ ok: true });
});
...
router.get('/clinics-overview', async (req, res) => {
    if (req.user.role !== 'superadmin') {
        res.status(403).json({ error: 'Forbidden' });
        return;
    }
    try {
        const orgs = await (0, db_1.default) `
    ...
    `;
        ...
        const ownerIds = orgs.map((o) => o.owner_id).filter((x) => x != null);
        const members = await (0, db_1.default) `
    ...
    `;
        ...
        const invited = ownerIds.length ? await (0, db_1.default) `
    ...
    ` : [];
        const result = orgs.map((o) => {
            const mem = members.filter((m) => m.org_id === o.id);
            const memIds = new Set(mem.map((m) => m.id));
            const pend = invited.filter((iv) => iv.owner_id === o.owner_id && !memIds.has(iv.id));
            const all = [...mem, ...pend];
            const counts = {};
            for (const m of all) {
...
async function getClinicAuditLog(req, res) {
...
router.get('/analytics/live', async (_req, res) => {
    try {
        const [now] = await (0, db_1.default) `
      SELECT
      ...
      FROM page_events`;
        const online = await (0, db_1.default) `
      ...
      GROUP BY user_id ORDER BY last_seen DESC LIMIT 50`;
        const recent = await (0, db_1.default) `
...
router.get('/analytics/engagement', async (_req, res) => {
    try {
        const [active] = await (0, db_1.default) `
      SELECT
      ...
      FROM page_events WHERE user_id IS NOT NULL`;
        const trend = await (0, db_1.default) `
      ...
      ORDER BY d`;
        ...
        console.error('[analytics/engagement]', e);
        res.json({ dau: 0, wau: 0, mau: 0, stickiness: 0, trend: [] });
    }
});
// Feature usage — which features get used, over 1d / 7d / 30d.
router.get('/analytics/features', async (_req, res) => {
    try {
        const rows = await (0, db_1.default) `
      SELECT event_type,
             COUNT(*)                                                          AS total,
...
router.get('/analytics/user/:id', async (req, res) => {
    try {
        ...
        const [summary] = await (0, db_1.default) `
      ...
      WHERE user_id = ${id} ORDER BY created_at DESC LIMIT 200`;
        const sessions = await (0, db_1.default) `
      ...
      FROM login_sessions WHERE user_id = ${id} ORDER BY logged_in_at DESC LIMIT 20`;
        ...
        res.json({ summary: {}, features: [], timeline: [], sessions: [] });
    }
});
// Errors / user issues — recent errors grouped by message, plus latest occurrences.
router.get('/analytics/errors', async (_req, res) => {
    try {
        const grouped = await (0, db_1.default) `
...
router.get('/analytics/devices', async (_req, res) => {
    try {
        const browsers = await (0, db_1.default) `
      SELECT CASE
      ...
      FROM page_events WHERE user_agent IS NOT NULL AND created_at > NOW() - INTERVAL '30 days'
      GROUP BY 1 ORDER BY events DESC`;
        const device = await (0, db_1.default) `
...
router.get('/analytics/growth-funnel', async (_req, res) => {
    try {
        const [u] = await (0, db_1.default) `
      SELECT
        COUNT(*)                                                          AS signups,
      ...
      FROM users WHERE role != 'superadmin'`;
        const [activated] = await (0, db_1.default) `
      SELECT COUNT(DISTINCT doctor_id) AS activated FROM visits`;
        const signupTrend = await (0, db_1.default) `
      ...
      LEFT JOIN (
        ...
        console.error('[analytics/growth-funnel]', e);
        res.json({ signups: 0, approved: 0, activated: 0, signupTrend: [] });
    }
});
...
router.get('/feedback', async (req, res) => {
    try {
        ...
        const rows = status
            ? await (0, db_1.default) `SELECT * FROM feedback WHERE status = ${status} ORDER BY created_at DESC LIMIT 300`
            : await (0, db_1.default) `SELECT * FROM feedback ORDER BY created_at DESC LIMIT 300`;
        res.json(rows);
    }
    catch (e) {
...
router.patch('/partner-applications/:id', async (req, res) => {
    try {
        const id = Number(req.params.id);
