...
exports.waNewBookingDoctor = waNewBookingDoctor;
exports.waBookingConfirmedPatient = waBookingConfirmedPatient;
...
function sendWhatsApp(toPhone, template, params) {
    if (!configured) {
        console.warn(`💬 skipped (unconfigured): ${template} → ${toPhone}`);
        return;
    }
    const to = toWaNumber(toPhone);
    if (!to) {
        console.warn(`💬 skipped (bad number): ${template} → ${toPhone}`);
        return;
    }
...
function waNewBookingDoctor(doctorPhone, d) {
...
function waBookingConfirmedPatient(patientPhone, d) {
