...
async function notify(userId: number, type: string, title: string, message: string, entityType: string, entityId: string) {
  await sql`
...
router.get('/', async (req: Request, res: Response) => {
  ...
  try {
    const rows = await sql`
      ...
      WHERE (
        ${direction === 'sent' ? sql`r.referring_doctor_id = ${userId}` :
          direction === 'received' ? sql`r.receiving_doctor_id = ${userId}` :
          sql`(r.referring_doctor_id = ${userId} OR r.receiving_doctor_id = ${userId})`}
      )
...
router.post('/', async (req: Request, res: Response) => {
  ...
  const {
    receivingDoctorId, patientId, patientName, patientAge, patientGender,
    patientPhone, reason, notes, clinicalInfo, urgency,
  } = req.body;
  ...
  try {
    ...
    const [recDoc] = await sql`
      SELECT id, name, specialty FROM users
      WHERE id = ${Number(receivingDoctorId)} AND approval_status = 'approved'
    `;
    if (!recDoc) return res.status(404).json({ error: 'Receiving doctor not found' });

    const [refDoc] = await sql`SELECT name FROM users WHERE id = ${referringDoctorId}`;
    ...
    await notify(
      Number(receivingDoctorId),
      'referral_received',
      'Patient Referral',
      `Dr. ${refDoc?.name ?? 'A colleague'} has referred a patient to you: ${patientName.trim()}. Reason: ${reason.trim().slice(0, 100)}`,
      'referral',
      String(referral.id),
    );
...
router.patch('/:id', async (req: Request, res: Response) => {
  const userId = req.user!.userId;
  const referralId = Number(req.params.id);
  const { action, declinedReason } = req.body as { action: 'accept' | 'decline' | 'cancel'; declinedReason?: string };
  ...
  try {
    ...
    if (action === 'accept') {
      ...
      const [recDoc] = await sql`SELECT name FROM users WHERE id = ${userId}`;
      await notify(
        Number(referral.referring_doctor_id), 'referral_accepted', 'Referral Accepted',
        `Dr. ${recDoc?.name ?? 'The doctor'} has accepted your referral for ${referral.patient_name as string}.`,
      ...
      const [recDoc] = await sql`SELECT name FROM users WHERE id = ${userId}`;
      await notify(
        Number(referral.referring_doctor_id), 'referral_declined', 'Referral Declined',
        `Dr. ${recDoc?.name ?? 'The doctor'} has declined your referral for ${referral.patient_name as string}.${declinedReason ? ` Reason: ${declinedReason}` : ''}`,
        'referral', String(referralId),
      );
    } else {
      [updated] = await sql`
